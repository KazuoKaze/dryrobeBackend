...
from .models import JustSending, Coupon, SizeVariant, ColorVariant
from .serializers import JustSendingSerializer, OrderItemSerializer, CouponSerizlier, SizeVariantSerizlier, ColorVariantSerizlier
...
def getSingleProduct(request, pk):
    ...
    variants = SizeVariant.objects.filter(product=product)
    colorVariants = ColorVariant.objects.filter(product=product)
    variant_serializer = SizeVariantSerizlier(variants, many=True)
    colorVariants_serializer = ColorVariantSerizlier(colorVariants, many=True)

    # Combine the product and variant data
    serialized_data = {
        'product': product_serializer.data,
        'variants': variant_serializer.data,
        'colorVariants': colorVariants_serializer.data
    }
...
def postOrderItem(request):
    if request.data:
        ...
        #     if idx < len(size_data) and idx < len(color_data) and idx < len(quantity_data):
        #         order_item_product = {
        #             'product': product_id,
        #             'size': size_data[idx],
        ...
        min_length = min(len(products_data), len(size_data), len(color_data), len(quantity_data))

        # Loop through products and colors simultaneously
        for idx, (product_id, color_id) in enumerate(zip(products_data, color_data)):
            if idx < min_length:  # Ensure all lists are processed until the shortest one
                order_item_product = {
                    'product': product_id,
                    'size': size_data[idx],
                    'color': color_id,
